...
SyringeDeviceSim::SyringeDeviceSim(int device_num, DeviceEvent device_event)
{
	m_devicevalue.device_type = DEVICE_TYPE_SYRINGE;
	m_devicevalue.device_name = "SyringeDevice";
	m_devicevalue.device_num = device_num;
	m_deviceevent = device_event;
	SetupUi(this);
}

SyringeDeviceSim::~SyringeDeviceSim()
{

}

int SyringeDeviceSim::Open()
{
	return 0;
}

int SyringeDeviceSim::Close()
{
	return 0;
}

const DeviceValue& SyringeDeviceSim::GetDeviceValue()
{
	return m_devicevalue;
}

int SyringeDeviceSim::PumpBlood()
{
	return 0;
}

int SyringeDeviceSim::CheckError()
{
	return 0;
}

int SyringeDeviceSim::SetError(int error_code, bool clear)
{
	do
	{
		if (clear) //< 清除错误
...
int SyringeDeviceSim::SetState(int state)
{
...
int SyringeDeviceSim::SetOpera(int opera)
{
...
int SyringeDeviceSim::SetupUi(QWidget* hostWidget)
{
	if (hostWidget)
	{
		this->setWindowTitle(typeid((*this)).name());
		connect(this, &SyringeDeviceSim::SignalSetState, this, &SyringeDeviceSim::SlotSetState);
		connect(this, &SyringeDeviceSim::SignalSetError, this, &SyringeDeviceSim::SlotSetError);

		Qt::WindowFlags flags = Qt::Dialog;
		flags |= Qt::WindowMinMaxButtonsHint;
		flags |= Qt::WindowCloseButtonHint;
		setWindowFlags(flags);
		hostWidget->setStyleSheet("SyringeDeviceSim{border:1px solid green;}");
		...
		box_device_error->addItem(ErrorCodeStr(BLOODROBOT_ERROR_SYRINGEDEVICE_BOOT).c_str(), BLOODROBOT_ERROR_SYRINGEDEVICE_BOOT);
		box_device_error->addItem(ErrorCodeStr(BLOODROBOT_ERROR_SYRINGEDEVICE_PUMP).c_str(), BLOODROBOT_ERROR_SYRINGEDEVICE_PUMP);
		...
		box_device_opera->addItem(DeviceOperaStr(SYRINGEDEVICE_OPERA_UNKNOWN).c_str(), SYRINGEDEVICE_OPERA_UNKNOWN);
		box_device_opera->addItem(DeviceOperaStr(SYRINGEDEVICE_OPERA_PUMP).c_str(), SYRINGEDEVICE_OPERA_PUMP);
		box_device_opera->addItem(DeviceOperaStr(SYRINGEDEVICE_OPERA_PUMPPOS).c_str(), SYRINGEDEVICE_OPERA_PUMPPOS);
...
int SyringeDeviceSim::SlotStateChanged(const QString& state_text)
{
...
int SyringeDeviceSim::SlotOperaChanged(const QString& opera_text)
{
...
int SyringeDeviceSim::SlotErrorChanged(const QString& error_text)
{
...
int SyringeDeviceSim::SlotSetState(int state)
{
	m_box_state->setCurrentText(DeviceStateStr(state).c_str());
	return 0;
}

int SyringeDeviceSim::SlotSetError(int error)
{
...
void DeleteSyringeDeviceSim(ISyringeDevice* device)
{
	if (device)
	{
		delete dynamic_cast<SyringeDeviceSim*>(device);
	}
}