...
class RobotArmDeviceSim :
    public QDialog,
    public IRobotArmDevice
{
    Q_OBJECT

public:
    RobotArmDeviceSim(int device_num, DeviceEvent device_event);
    ~RobotArmDeviceSim();
...
public:
    ...
    virtual int MovePos(RobotArmDevicePos dest_pos) override;  // 控制机械臂
...
signals:
    int SignalMovePos(RobotArmDevicePos arm_pos);
private slots:
    ...
    int SlotMovePos(RobotArmDevicePos arm_pos);

private:
    RobotArmDeviceValue m_devicevalue;
    DeviceEvent m_deviceevent;
    QComboBox* m_box_state = nullptr;
    QComboBox* m_box_opera = nullptr;
    QComboBox* m_box_error = nullptr;
    vector<QSlider*>  m_armpos; //< 机械臂尖端(最后一个关节)位置
};
